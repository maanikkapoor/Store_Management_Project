		...
		if (choice==1){
			...
			for (unsigned short i=0;i<choice;++i){
				if (Data_Type[i]=="string"||Data_Type[i]=="char"||Data_Type[i]=="char16_t"||Data_Type[i]=="char32_t"||Data_Type[i]=="wchar_t")
					...
					unsigned __int64 limit;//datatype limit
...
protected:
	...
	static unsigned __int64 ID_Generator(const bool Reset_Flag=false){
	...
	virtual void Input_Data(istream&)=0;
...
class Items_List final{
	...
	void Add_or_Insert_Item(const unsigned short Ctgry_Choice,const bool Menu_Choice_Flag){
		...
		if (Menu_Choice_Flag&&Items_Counter>1){
	...
	bool Read_Data_Files(const string File_Name){
		...
		if(Temp_Items_Counter>Items_Size){
			if(!(Temp_Items_Counter%10))
				...
				Resize_Items("increased",10*((Temp_Items_Counter/10)+1));
		}
...
class Store final{
	...
	static void Password(string& Pass){
		...
		do{
			...
			else if(Temp_Pass[i]==BackSpace&&i!=0){
				cout<<"\b \b";
				i--;
			}
	...
	static void Add_New_Category(){
		...
		if(!Glbl::Check_Files(File_Names[0],File_Names[1],File_Names[2])){
		...
		system("g++ Store_Management.cpp -o Store_Management.exe -std=gnu++11"); 
	...
	static void Update_Current_DataBase(Items_List& List,const unsigned short DB_Index){
		...
		while(true){
			...
			switch (Menu_Choice){
				...
				case 10:
					Change_Category:
					for (unsigned short i=1;i<=Ctgry::Category_Count;++i)
	...
	static void Change_App_Name(string Path){
		reverse(Path.begin(),Path.end());
		string App_Name;
		for (unsigned short i=0;Path[i]!='\\';App_Name+=Path[i++]);
		reverse(App_Name.begin(),App_Name.end());
		if(App_Name=="Store_Management.exe"){
			remove("Store_Management.exe");
			rename(App_Name.c_str(),"Store_Management.exe");
		}
	}
	static bool Retailer_Menu(unsigned short& Data_Base_Counter,unsigned short& Customer_Data_Base_Index){
		...
		unsigned short Cur_DB_Index=0,Choice=0;
		string Cur_DB_Nme_Str,Temp_DB_Nme_Str;
		Items_List List;
		while(true){
			system("cls");
			unsigned short Cursor_Pos=1;
			if(Menu_Flag){
	...
	Save_Data_to_New_DB:
				...
				case 6:{
					...
					for(unsigned short i=1;i<=Data_Base_Counter;++i){
					...
					Cur_DB_Nme_Str="Data_Files/Data_Bases/Data_Base_"+to_string(Choice)+".txt";
					remove(Cur_DB_Nme_Str.c_str());
					for (unsigned short i=Choice;i<Data_Base_Counter;++i){
...
public:
	static void Main_Menu(string Path="NULL"){
